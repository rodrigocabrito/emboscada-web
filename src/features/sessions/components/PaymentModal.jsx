const PAYMENT_TYPES = [
  ...
  // darkIcon: the default MBWay logo is near-black and vanishes in dark mode
  { value: 'mbway', label: 'MBWay', icon: '/mbway.png', darkIcon: '/mbway-dark-mode.png' },
  { value: 'cash', label: 'Dinheiro', icon: '/cash.png' },
];

const ICON_STYLE = { width: '40px', height: '40px', objectFit: 'contain' };
          ...
          {PAYMENT_TYPES.map((pt) => {
            ...
            return (
              <button
                ...
                className={`pay-type-btn${selected ? ' is-selected' : ''}`}
              ...
              >
                <div style={{
                  width: '100%', height: '68px', padding: '0.85rem', boxSizing: 'border-box', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  ...
                  {pt.darkIcon ? (
                    <>
                      <img className="pay-icon--light" src={pt.icon} alt={pt.label} style={ICON_STYLE} />
                      <img className="pay-icon--dark" src={pt.darkIcon} alt={pt.label} style={ICON_STYLE} />
                    </>
                  ) : (
                    <img src={pt.icon} alt={pt.label} style={ICON_STYLE} />
                  )}
                </div>
                <span className="pay-type-label" style={{ fontSize: '0.75rem', fontWeight: 600 }}>{pt.label}</span>
              </button>
            );
          })}
        </div>
