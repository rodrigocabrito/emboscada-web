...
// "14:00" -> 840
const sessionMinutes = (t) => {
  if (!t) return null;
  const [h, m] = t.split(':');
  return parseInt(h, 10) * 60 + (parseInt(m, 10) || 0);
};
// "14h00" / "14h30" -> minutes
const availMinutes = (s) => {
  if (!s) return null;
  const [h, m] = s.split('h');
  return parseInt(h, 10) * 60 + (parseInt(m, 10) || 0);
};
const formatHour = (s) => (s ? (s.endsWith('h00') ? s.replace('h00', 'h') : s) : '');
...
const monitorAvail = (uid, availByUser) => {
...
// Scheduling rules
const SESSION_LEN = 120;     // every session is 2h long
const CHECKIN_BEFORE = 60;   // monitor checks in 1h before the start
const GAP_BETWEEN = 60;      // 1h interval between one session's end and another's start
const EXIT_BUFFER = 30;      // session must end at least 30min before the exit limit
...
const monitorWarnings = (avail, session, monitorUid, daySessions) => {
  const warnings = [];
